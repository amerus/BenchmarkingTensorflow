...
# Dedicated training on CPU. GPU version of the script is also available.
...
# Setting timeout vallue for network training time on CPU
RUNTIME='75m'
...
MODEL_DIR=$WHDIR/waldo/localTrainingCPU

# Setting pipeline file location for the model
PIPELINE=$WHDIR/waldo

# Removing logs from the previous run
if [ ! -z "$(ls $LOGDIR)" ]; then
...
# Training the network on CPU. Marking CUDA devices invisible and executing for a fixed amount of runtime.
...
# Saving process id of the training script into a variable
PSModel=$!
...
INFERENCE_DIR=$WHDIR/waldo/inferenceGraphCPU
...
# Waiting for all of data collection to complete before exporting the checkpoints into a inference graph
wait $PSloop
